...
```{r}
global <- select(processed, 31:36) %>% names()
met <- select(processed, 25:29) %>% names
...
correlation_res %>% 
  select(-pearson_res, -spearman_res) %>% 
...
To answer the question we first calculate the total score of the diener scale.

```{r}
diener_rezil_data <-
  processed %>% 
  mutate(diener_sum = diener1 + diener2 + diener3 + diener4 + diener5 + diener6 + diener7 + diener8) %>% 
  select(rezil, diener_sum)
```
